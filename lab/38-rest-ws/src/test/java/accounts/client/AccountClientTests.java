...
public class AccountClientTests {
	...
	public void addAndDeleteBeneficiary() {
		...
		long accountId = 1;
		URI locationDavid = restTemplate.postForLocation(BASE_URL + "/accounts/" + accountId + "/beneficiaries", "David");
		assertNotNull(locationDavid);

		// TODO-14: Retrieve the Beneficiary you just created from the location that was returned
		Beneficiary newBeneficiary = restTemplate.getForObject(locationDavid, Beneficiary.class);

		assertNotNull(newBeneficiary);
		assertEquals("David", newBeneficiary.getName());
		
		// TODO-15: Delete the newly created Beneficiary
		restTemplate.delete(locationDavid);
		...
		HttpClientErrorException httpClientErrorException = assertThrows(HttpClientErrorException.class, () -> {
			...
			//   was not successful.
			Beneficiary davidAgain = restTemplate.getForObject(locationDavid, Beneficiary.class);
			assertNull(davidAgain);
		});
